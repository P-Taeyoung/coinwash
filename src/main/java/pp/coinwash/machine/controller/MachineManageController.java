...
public class MachineManageController {
	...
	@PostMapping
	public ResponseEntity<String> registerMachine(List<MachineRegisterDto> dtos,long laundryId, long ownerId) {
	...
	@GetMapping
	public ResponseEntity<List<MachineResponseDto>> getMachines(long laundryId, long ownerId) {
		return ResponseEntity.ok(machineManageService.getMachinesByLaundryId(laundryId, ownerId));
	}

	@PatchMapping
	public ResponseEntity<String> updateMachine(MachineUpdateDto dto, long ownerId) {
		machineManageService.updateMachine(dto, ownerId);
		return ResponseEntity.ok("기계정보가 정상적으로 수정되었습니다.");
	}

	@DeleteMapping
	public ResponseEntity<String> deleteMachine(long laundryId, long ownerId) {
